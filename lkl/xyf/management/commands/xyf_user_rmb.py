    ...
    def handle(self, start, end, *args, **options):
        ...
        for obj in objs:
            if obj.trans_id in used_trans_ids:
                continue
            adatetime = utils.string_to_datetime(obj.trade_date, format_str="%Y%m%d %H%M%S")
...
def process_xyf_rmb(obj, default_user):
    ...
    if hasattr(user, "xyffenrun"):
        ...
        return
    armb = (Decimal("0.550") - Decimal(user_point)) / Decimal("0.035") * Decimal(obj.fee_rmb) * Decimal("0.92")
