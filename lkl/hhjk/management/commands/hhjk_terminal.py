...
import time
from hhjk.models import HHJKTerminal
...
def write_to_db_terminal(data):
    ...
    used_tids = set(HHJKTerminal.objects.filter(sn_code__in=tids).values_list("sn_code", flat=True))
    # 插入db
    alist = []
    same_code = set()
    for t in data:
        ...
        if t[6] not in used_tids:
            ...
            itmes = HHJKTerminal.objects.filter(sn_code=t[6])
            if itmes:
                item = itmes[0]
                item.factory = t[0]
                ...
                item.save()
    if alist:
        HHJKTerminal.objects.bulk_create(alist)
