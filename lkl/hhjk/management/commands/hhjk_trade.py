...
from hhjk.models import HHJKTrade
...
def write_to_db_trade(data):
    ...
    used_tids = set(HHJKTrade.objects.filter(trans_id__in=tids).values_list("trans_id", flat=True))
    # 插入db
    alist = []
    same_code = set()
    for t in data:
        ...
        if trans_id not in used_tids:
            ...
            itmes = HHJKTrade.objects.filter(trans_id=t[17])
            if itmes:
                item = itmes[0]
                ...
                item.save()
    if alist:
        HHJKTrade.objects.bulk_create(alist)
