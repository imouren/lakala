...
from hhjk.models import HHJKMerchant
from user.utils import wrapper_raven
from hhjk.dbutils import get_token_code, disable_token
...
def write_to_db_merchant(data):
    ...
    used_tids = set(HHJKMerchant.objects.filter(merchant_code__in=tids).values_list("merchant_code", flat=True))
    # 插入db
    alist = []
    same_code = set()
    for t in data:
        ...
        if t[0] not in used_tids:
            ...
            itmes = HHJKMerchant.objects.filter(merchant_code__in=t[0])
            if itmes:
                item = itmes[0]
                ...
                item.save()
    if alist:
        HHJKMerchant.objects.bulk_create(alist)
